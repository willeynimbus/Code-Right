...
type ProblemDiscriptionProps = {
  problem: Problem;
  _solved: boolean;
};

const ProblemDiscription: React.FC<ProblemDiscriptionProps> = ({
  problem,
  _solved,
}) => {
  ...
  const returnUserDataAndProblemData = async (transaction: any) => {
    ...
    const userDoc = await transaction.get(problemRef);
    const problemDoc = await transaction.get(problemRef);
    return { userDoc, problemDoc, userRef, problemRef };
  };
  const handleLike = async () => {
    if (!user) {
      toast.error("You must me logged in a to like a problem", {
        position: "top-left",
        theme: "dark",
      });
      return;
  ...
  const handleDislike = async () => {
    if (!user) {
      toast.error("You must me logged in a to dislike a problem", {
        position: "top-left",
        theme: "dark",
      });
      return;
    ...
    await runTransaction(firestore, async (transaction) => {
      ...
      if (userDoc.exists() && problemDoc.exists()) {
        if (disliked) {
          ...
          setCurrentProblem((prev) =>
            prev
              ? { ...prev, disliles: prev.dislikes + 1, likes: prev.likes - 1 }
              : null
          );
